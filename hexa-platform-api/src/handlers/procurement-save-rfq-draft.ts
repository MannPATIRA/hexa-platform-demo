...
async function getGraphAccessToken() {
  ...
  if (!tokenRes.ok) {
    ...
    if (raw.includes("AADSTS70000")) {
      throw new Error(
        "Draft permission is missing. Add Microsoft Graph delegated Mail.ReadWrite permission, then re-consent to the app to refresh MS_GRAPH_REFRESH_TOKEN."
      );
    }
...
export async function handleProcurementSaveRfqDraft(c: Context) {
    ...
    return c.json(
      {
        ...
        message: "Set PROCUREMENT_EMAIL in .env to define which Microsoft mailbox saves RFQ drafts.",
      },
      503
    );
  }
  ...
  const { rfqRef, itemName, supplierEmail } = payload;
  ...
  try {
    ...
    if (!draftRes.ok) {
      const raw = await draftRes.text();
      throw new Error(`Graph draft create failed: ${raw}`);
    }

    const draft = (await draftRes.json()) as GraphDraftResponse;
    return c.json({
      success: true,
      message: `RFQ draft ${rfqRef} saved in ${email} Drafts folder for recipient ${recipientEmail}`,
      rfqRef,
      to: recipientEmail,
      supplierEmail,
    ...
    return c.json({ error: "Draft save failed", message }, 500);
  }
}
