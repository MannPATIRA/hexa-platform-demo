...
import type { RiskTier, Recommendation, Status } from "@/lib/mockData";
...
function StatusLabel({ status }: { status: Status }) {
  ...
  return (
    <span className={cn("text-[13px]", status === "OPEN" ? "text-primary" : "text-muted-foreground")}>
      {labels[status]}
    </span>
  );
}
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <h1 className="text-[28px] font-normal text-foreground font-[family-name:var(--font-serif)]">
                {supplier.name}
              </h1>
